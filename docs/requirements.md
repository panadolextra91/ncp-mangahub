...
Project Deliverables
Source code and documentation must be submitted on Blackboard before the due date. Zip all files and name it GroupXX_MangaHub.zip (e.g., Group01_MangaHub.zip)
A demonstration session will be held at the end of the course showing all five protocols working together
Fail to show up during the demonstration session will result in ZERO grading for the project
Due Date
...
Simple API Integration: 100 additional series from MangaDex API (or other legal APIs) using basic calls
...
Simple MangaDex API integration
...
Completed one or more random bonus features to fulfil the remaining 10 points.
